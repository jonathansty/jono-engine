...
AudioDecoder::AudioDecoder(const std::wstring& filenameRef, XAUDIO2_BUFFER& bufferRef, WAVEFORMATEXTENSIBLE* pWfx)
{
	...
	if (FAILED(hr))
	{
		assert("Error Loading sound.GameEngine says NO");
		exit(-1);
	}
}
...
HRESULT AudioDecoder::Open(const std::wstring& wFilenameRef, XAUDIO2_BUFFER& bufferRef, WAVEFORMATEXTENSIBLE* pWfx)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		cbMaxAudioData = (DWORD)((durationInNanoSecs / 10000000 + 1) * pWav->nSamplesPerSec * pWav->wBitsPerSample / 8 * pWav->nChannels);
