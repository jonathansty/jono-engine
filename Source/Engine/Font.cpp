...
{
	std::wstring str(fontName.begin(), fontName.end());
...
void Font::LoadTextFormat(const wchar_t* fontName, float size)
{
	...
	hr = GameEngine::instance()->GetDWriteFactory()->CreateTextFormat(fontName, NULL, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, size, L"", &m_TextFormatPtr);
