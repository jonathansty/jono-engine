...
Lever::Lever(float2 position, Bitmap* bmpPtr):
Entity(position),
m_BmpPtr(bmpPtr)
{
...
void Lever::Paint(graphics::D2DRenderContext& ctx)
{
    ...
    matPivot= float3x3::translation(float2(-(m_BmpPtr->GetWidth()/AMOUNTOFFRAMES) / 2, -m_BmpPtr->GetHeight() / 2));
...
void Lever::SetPosition(float2 position)
{
