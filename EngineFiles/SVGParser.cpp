...
bool SVGParser::LoadGeometryFromSvgFile(const String& svgFilePathRef, std::vector<std::vector<DOUBLE2>> &verticesArrRef)
{
	...
	if (!svgFile)
	{
		game_engine::instance()->MessageBox(String("Failed to load geometry from file ") + svgFilePathRef);
		return false;
	}
	else
	{
		...
		if (result == false)
		{
			game_engine::instance()->MessageBox(String("Malformed geometry information in file ") + svgFilePathRef);
			return false;
		}
...
bool SVGParser::ReadSvgFromData(tstring& sTransformInfoRef, tstring& sPathInfoRef, std::vector<std::vector<DOUBLE2>> &verticesArrRef)
{
	...
	if (!result)
	{
		game_engine::instance()->MessageBox(String("SVG path read failed"));
		return false;
	}
	return true;
}
...
bool SVGParser::GetGeometryOrGroup(ID2D1Factory* factoryPtr, std::vector<ID2D1Geometry*>& childrenRef, ID2D1Geometry*& geometryPtrRef)
{
	if (!childrenRef.empty())
	{
		...
		else
		{
			...
			if (FAILED(hr))
			{
				game_engine::instance()->MessageBox(String("CreateTransformedGeometry failed"));
				return false;
			}
...
MATRIX3X2 SVGParser::ReadTransform(tstring& sTransformInfoRef)
{
	...
	if (sTransformInfoRef != _T(""))
	{
		...
		default:
			game_engine::instance()->MessageBox(String("unsupported transform attribute"));
			break;
		}
	}
...
bool SVGParser::ReadSvgPath(tstring& sPathInfoRef, std::vector<std::vector<DOUBLE2>> &verticesArrRef)
{
	...
	while (true)
	{
		...
		case _T('C'):
		case _T('c'):
			game_engine::instance()->MessageBox(String(cmd) + String(" Beziers are not supported. Have another look at the guide, or select all nodes in inkscape and press shift + L"));
			break;
		...
		default:
			game_engine::instance()->MessageBox(String(cmd) + String(" is not a supported SVG command"));
			//return false;
		}
	}
...
void SVGParser::SkipSvgComma(tstringstream& ssRef, bool isRequired)
{
	while (true)
	{
		...
		if (ssRef.eof())
		{
			if (isRequired) game_engine::instance()->MessageBox(String("expected comma or whitespace"));
			break;
		}
...
{
	if (!isOpen) game_engine::instance()->MessageBox(String("expected 'Z' or 'z' command"));
...
{
	if (isOpen) game_engine::instance()->MessageBox(String("expected 'M' or 'm' command"));
...
{
	if (isOpen)
		game_engine::instance()->MessageBox(String("expected 'M' or 'm' command"));
