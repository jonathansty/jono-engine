...
void Box2DDebugRenderer::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
	...
	game_engine::instance()->SetColor(COLOR((byte)color.r, (byte)color.g, (byte)color.b, m_Color.alpha));
...
void Box2DDebugRenderer::DrawTransform(const b2Transform& xf)
{
	...
	game_engine::instance()->SetColor(COLOR(255,0,187));
	...
	game_engine::instance()->SetColor(COLOR(0,255,187));
